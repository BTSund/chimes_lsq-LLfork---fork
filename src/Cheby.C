			 ...
			 if(rlen > FF_2BODY[curr_pair_type_idx].S_MINIM and rlen < FF_2BODY[curr_pair_type_idx].S_MAXIM)
			 {
				 ...
				 for ( int i=0; i<FF_2BODY[curr_pair_type_idx].SNUM; i++ ) 
				 {
					for ( int j=0; j <= CONTROLS.ALCH_2B_ORDER-1; j++)
					{
						...
						// cout << "index: " << vstart+i*CONTROLS.ALCH_2B_ORDER+j << endl;
						A_MATRIX.FORCES[a1     ][vstart+i*CONTROLS.ALCH_2B_ORDER+j].X += deriv;
						A_MATRIX.FORCES[fidx_a2][vstart+i*CONTROLS.ALCH_2B_ORDER+j].X -= deriv;

						deriv = tmp_doub * RAB.Y / rlen; 
						A_MATRIX.FORCES[a1     ][vstart+i*CONTROLS.ALCH_2B_ORDER+j].Y += deriv;
						A_MATRIX.FORCES[fidx_a2][vstart+i*CONTROLS.ALCH_2B_ORDER+j].Y -= deriv;

						deriv = tmp_doub * RAB.Z / rlen;
						A_MATRIX.FORCES[a1     ][vstart+i*CONTROLS.ALCH_2B_ORDER+j].Z += deriv;
						A_MATRIX.FORCES[fidx_a2][vstart+i*CONTROLS.ALCH_2B_ORDER+j].Z -= deriv;

						if (CONTROLS.FIT_STRESS)
						{
							A_MATRIX.STRESSES[vstart+i*CONTROLS.ALCH_2B_ORDER+j].XX -= tmp_doub * RAB.X * RAB.X / rlen;
							A_MATRIX.STRESSES[vstart+i*CONTROLS.ALCH_2B_ORDER+j].YY -= tmp_doub * RAB.Y * RAB.Y / rlen;
							A_MATRIX.STRESSES[vstart+i*CONTROLS.ALCH_2B_ORDER+j].ZZ -= tmp_doub * RAB.Z * RAB.Z / rlen;      
						}

						else if (CONTROLS.FIT_STRESS_ALL)
						{
							A_MATRIX.STRESSES[vstart+i*CONTROLS.ALCH_2B_ORDER+j].XX -= tmp_doub * RAB.X * RAB.X / rlen;   // xx
							A_MATRIX.STRESSES[vstart+i*CONTROLS.ALCH_2B_ORDER+j].XY -= tmp_doub * RAB.X * RAB.Y / rlen;   // xy
							A_MATRIX.STRESSES[vstart+i*CONTROLS.ALCH_2B_ORDER+j].XZ -= tmp_doub * RAB.X * RAB.Z / rlen;   // xz

							A_MATRIX.STRESSES[vstart+i*CONTROLS.ALCH_2B_ORDER+j].YY -= tmp_doub * RAB.Y * RAB.Y / rlen;   // yy
							A_MATRIX.STRESSES[vstart+i*CONTROLS.ALCH_2B_ORDER+j].YZ -= tmp_doub * RAB.Y * RAB.Z / rlen;   // yz
							A_MATRIX.STRESSES[vstart+i*CONTROLS.ALCH_2B_ORDER+j].ZZ -= tmp_doub * RAB.Z * RAB.Z / rlen;   // zz
						}
